...
(function (window) {
  ...
  // ---------- standardize tab ----------
  var stdout_ = document.getElementById("stdout");
  document.getElementById("stdapply").onclick = function () {
    var btn = this;
    btn.disabled = true;
    var pending = bubble(stdout_, "sys", "Applying standard…");
    // file name for the header comes from the plugin runtime info
    ...
    window.Asc.plugin.callCommand(function () {
      ...
      try {
        ...
        // 1. style definitions: body + headings
        function styleText(styleName, font, sizePt, color, bold) {
          try {
            var st = doc.GetStyle(styleName);
            if (!st) return;
            var pr = st.GetTextPr();
            pr.SetFontFamily(font);
            pr.SetFontSize(sizePt * 2);           // half-points
            pr.SetColor(color[0], color[1], color[2]);
            if (bold !== undefined) pr.SetBold(bold);
            result.push(styleName);
          } catch (e) { result.push(styleName + "!" + e.message); }
        }
        styleText("Normal",    "Calibri", 11, [29, 29, 31]);
        styleText("Heading 1", "Calibri", 16, [0, 121, 193], true);
        styleText("Heading 2", "Calibri", 14, [0, 121, 193], true);
        styleText("Heading 3", "Calibri", 12, [0, 98, 155], true);

        // 2. header: file name (right-aligned, small grey)
        var section = doc.GetFinalSection();
        var header = section.GetHeader("default", true);
        header.RemoveAllElements();
        var hp = header.GetElement(0) || Api.CreateParagraph();
        hp.RemoveAllElements();
        hp.SetJc("right");
        var hr = Api.CreateRun();
        hr.AddText(Asc.scope.docname);
        var hpr = hr.GetTextPr(); hpr.SetFontSize(18); hpr.SetColor(110, 110, 115); hpr.SetFontFamily("Calibri");
        hp.AddElement(hr);
        if (!header.GetElement(0)) header.Push(hp);
        result.push("header");

        // 3. footer: page number, centred
        var footer = section.GetFooter("default", true);
        footer.RemoveAllElements();
        var fp = footer.GetElement(0) || Api.CreateParagraph();
        fp.RemoveAllElements();
        fp.SetJc("center");
        try {
          fp.AddPageNumber();
          var fr = Api.CreateRun(); // style the number via paragraph text props where supported
          result.push("footer:pagenum");
        } catch (e) {
          var fr2 = Api.CreateRun(); fr2.AddText("Page ");
          fp.AddElement(fr2);
          result.push("footer:text-fallback");
        }
        if (!footer.GetElement(0)) footer.Push(fp);
      ...
      pending.remove();
      if (String(r).indexOf("ERROR") === 0) {
        bubble(stdout_, "sys", "Could not apply: " + r);
      } else {
        bubble(stdout_, "ai", "Standard applied — body and heading styles, header with file name, page numbers in the footer. Review the document; the previous state is in version history.");
      }
      btn.disabled = false;
    });
  };
